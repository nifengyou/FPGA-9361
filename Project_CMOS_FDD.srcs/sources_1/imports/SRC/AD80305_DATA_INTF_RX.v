...
reg     [11:0]  Data_in_i_tmp_1;
reg     [11:0]  Data_in_q_tmp_1;
wire    [25:0]  Data_fifo_out;
/*****************************************************/
/*-------               Main Code            --------*/
/*****************************************************/
...
always @ ( posedge Clk )
begin
    Data_in_i_tmp_1 <= Data_in_i_tmp;
    Data_in_q_tmp_1 <= Data_in_q_tmp;
end
...
begin
    ...
    begin
        Data_in_i <= Data_in_i_tmp_1;
        Data_in_q <= Data_in_q_tmp_1;
    end
    else
    begin
        Data_in_i <= 12'b0;
...
INTF_FIFO U_INTF_FIFO (
  ...
  .din      ( data_in_to_device             ),  // input wire [25 : 0] din
  .wr_en    ( 1'b1                          ),  // input wire wr_en
  .rd_en    ( 1'b1                          ),  // input wire rd_en
  .dout     ( Data_fifo_out                 ),  // output wire [25 : 0] dout
...
always @ ( posedge Clk )
begin
    if ( ( Data_fifo_out[25] == 1'b1 ) && ( Data_fifo_out[12] == 1'b0 ) )
        Data_in_q_tmp <= Data_fifo_out[11:0];
    else if ( ( Data_fifo_out[25] == 1'b0 ) && ( Data_fifo_out[12] == 1'b1 ) )
        Data_in_q_tmp <= Data_fifo_out[24:13];
    else
        Data_in_q_tmp <= 12'b0;
end
...
begin
    if ( ( Data_fifo_out[25] == 1'b1 ) && ( Data_fifo_out[12] == 1'b0 ) )
        Data_in_i_tmp <= Data_fifo_out[24:13];
    else if ( ( Data_fifo_out[25] == 1'b0 ) && ( Data_fifo_out[12] == 1'b1 ) )
        Data_in_i_tmp <= Data_fifo_out[11:0];
    else
        Data_in_i_tmp <= 12'b0;
end
